...
namespace Engine::Components {
  ...
  public:
    ...
    MeshRenderer()
        : Component("Mesh Renderer", ECS::GetComponentTypeID< MeshRenderer >()), _dirty(true) {
    }
    MeshRenderer(std::shared_ptr< Renderer::Model > model, int meshIndex,
                 std::shared_ptr< Renderer::Material > material)
        : Component("Mesh Renderer", ECS::GetComponentTypeID< MeshRenderer >()), _model(model),
          _material(material), _meshIndex(meshIndex), _dirty(true) {
    }
    MeshRenderer(std::shared_ptr< Renderer::Model > model,
                 std::shared_ptr< Renderer::Material > material)
        : Component("Mesh Renderer", ECS::GetComponentTypeID< MeshRenderer >()), _model(model),
          _material(material), _meshIndex(0), _dirty(true) {
    }
