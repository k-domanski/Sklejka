...
StartTimer::StartTimer(std::shared_ptr< PlayerRect > playerRect, std::shared_ptr< FlightTimer > flightTimer): Script(), _time(0.0f), _precision(0), _playerRect(playerRect), _flightTimer(flightTimer) {
...
auto StartTimer::Update(float deltaTime) -> void {
  if (_canCount) {
    ...
    _stream << std::fixed << std::setprecision(_precision) << 5 - _time;
